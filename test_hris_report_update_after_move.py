"""
Temporary test file for Sprint 6.19 HRIS Upload Report Update After Move.

Run from project root:
py test_hris_report_update_after_move.py
"""
...
TEMP_CONFIG_FILE = Path(
    r"D:\Python Project\OfficeAutomationSuite-Karina\temp\OAS-K_HRIS_Configuration_Report_Update_Test.xlsx"
)

SAMPLE_TXT_FOLDER = Path(
    r"D:\Python Project\OfficeAutomationSuite-Karina\temp\hris_txt_sample_report_update"
)
...
MOCK_UPLOAD_FILE = MOCK_SITE_FOLDER / "report_update_local.html"
...
<head>
    <meta charset="UTF-8">
    <title>Mock HRIS Report Update - OAS-K</title>
    <style>
...
def main() -> None:
    print("=" * 80)
    print("OAS-K Sprint 6.19 HRIS Report Update After Move Test")
    print("=" * 80)
    ...
    try:
        ...
        print()
        print("REPORT UPDATE RESULT")
        print("-" * 80)
        print(f"Report File   : {report_file}")
        print(f"Report Exists : {report_file.exists()}")
        print(f"Job Status    : {sheet['B9'].value}")

        print()
        print("REPORT DETAIL CHECK")
        print("-" * 80)

        for row in range(13, 16):
            txt_file = sheet.cell(row=row, column=3).value
            status = sheet.cell(row=row, column=7).value
            message = sheet.cell(row=row, column=8).value
            moved_to = sheet.cell(row=row, column=9).value
    ...
    print()
    print("=" * 80)
    print("HRIS Report Update After Move test finished.")
    print("=" * 80)
