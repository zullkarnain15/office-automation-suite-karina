"""
Temporary test file for Sprint 5.5 Attendance Validation Engine.
...
def main() -> None:
    ...
    print("=" * 80)
    print("OAS-K Attendance Validation Engine Test")
    print("=" * 80)
    ...
    print(f"Total validated records      : {summary['total_records']}")
    print(
        "Valid for TXT records        : "
        f"{summary['valid_for_txt_records']}"
    )
    print(
        "Valid with warning records   : "
        f"{summary['valid_with_warning_records']}"
    )
    print(f"Anomaly records              : {summary['anomaly_records']}")
    ...
    print(
        "Missing check in records     : "
        f"{summary['missing_check_in_records']}"
    )
    print(
        "Missing check out records    : "
        f"{summary['missing_check_out_records']}"
    )
    ...
    for record in valid_records[:20]:
        print(
            {
                "nik": record["nik"],
                ...
                "validation_remarks": record["validation_remarks"],
            }
        )
    ...
    for record in anomaly_records[:20]:
        print(
            {
                "nik": record["nik"],
                "attendance_date": record["attendance_date"],
